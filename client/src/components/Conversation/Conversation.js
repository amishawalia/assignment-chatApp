...
/*   function stringAvatar(name) {
  ...
  return <>
    <div onClick={() => {
      ...
      retainOldMessages(obj._id, obj.admin ? true : false);
      // setTimeout(() => document.getElementById("send-message").focus(), 1000);
    }}
      className='card'
      tabIndex="0"
    >
      <div className={active == obj._id? "card_item active": "card_item"}  >
        ...
        <div className="card_item-info">
          ...
          {/* {obj.lastMessage ? (<> */}
            ...
            <div>
              <h3>{obj.userName}</h3>
              <p>{obj?.lastMessage?.message}</p>
            </div>
            <div className="card_item-info_meta">
              {obj?.lastMessage?.timestamp && <p>{getTimeToDisplayOnChat(obj?.lastMessage?.timestamp)}</p>}
